	...
	methods : {
	    selectCases:function(selects){
		 ...
		 load:function(page,size){//查询
			...
			zs_post({
				url: '../../venuesbook/comactivityentry/list?_' + $.now(),
				param:th.param,
				success:function(r){
					console.log(r);
		...
		save: function() {
			dialogOpen({
				title: '新增活动报名记录表',
				url: 'venue/comactivityentry/add.html?_' + $.now(),
				width: '40%',
				height: '80%',
		...
		edit: function(row) {
			...
			if(checkedRow(ck)){
				dialogOpen({
					title: '编辑活动报名记录表',
					url: 'venue/comactivityentry/edit.html?_' + $.now(),
					width: '40%',
					height: '80%',
					success: function(iframeId){
		...
		remove: function(row) {
			...
			if(checkedArray(ck)){
				...
				$.RemoveForm({
					url: '../../venuesbook/comactivityentry/remove?_' + $.now(),
			    	param: ids,
